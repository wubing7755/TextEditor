...
Add implementation files under `src/texteditor/` and public headers under
`include/texteditor/`.
...
Use `texteditor_add_test()` from `cmake/Tests.cmake`:

```cmake
texteditor_add_test(texteditor_feature_tests
    SOURCES
...
```sh
cmake -G Ninja -S tests/subproject_smoke -B build/subproject-smoke -DTEXTEDITOR_SOURCE_DIR="$PWD"
...
`find_package(TextEditor)` cannot find the package:

- Symptom: CMake cannot find `TextEditorConfig.cmake`.
